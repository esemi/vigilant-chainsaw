...
from bot.fishing.cv_helpers import looking_for_nibbles, search_bobber, show_current_frame
...
def test_looking_for_nibbles_happy_path_first(screenshots_dir: Path, fixture_farm_state: State,
...
def test_looking_for_nibbles_false_positive(screenshots_dir: Path, fixture_farm_state: State,
                                            frame_image_path: str):
    screenshot = cv2.imread(str(screenshots_dir / frame_image_path), cv2.IMREAD_GRAYSCALE)
    gray_frame = numpy.array(screenshot)
    area = search_bobber(fixture_farm_state, gray_frame, Point(0, 0))[0]
